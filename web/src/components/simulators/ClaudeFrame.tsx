function ClaudeBubble({ role, text, title, brand }) {
  if (role === 'toolAction') {
    return (
      <div className="rounded-xl border border-dashed bg-teal-50 p-3 text-xs text-slate-700" style={{ borderColor: brand.accentColor }}>
  ...
  return (
    <section className="flex h-full min-h-[480px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <header className="border-b bg-slate-50 px-4 py-3" style={{ borderColor: brand.accentColor }}>
        <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Claude</p>

...
export default function AgentsList({ selectedAgent, setSelectedAgent }) {
  return (
    <div className="flex-1 overflow-y-auto px-2">
      <h4 className="text-[12px] font-semibold text-[#616161] uppercase tracking-wide px-2 mb-1.5 mt-1">Agents</h4>
      {agents.map((agent) => (
        <button
          key={agent.id}
          onClick={() => setSelectedAgent(agent.id)}
          className={`
            w-full flex items-center gap-2.5 px-2.5 py-[7px] rounded-md text-left transition-colors mb-[1px]
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[12px] text-[#616161] truncate">{agent.description}</p>
          </div>
